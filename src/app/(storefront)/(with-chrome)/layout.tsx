...
import { normalizeCanonicalOrigin } from "../../../lib/utils";
...
export default async function StorefrontChromeLayout({ children }: Props): Promise<JSX.Element> {
  ...
  const rawSiteUrl =
    (websiteSettings.canonicalBaseUrl || websiteSettings.siteUrl).replace(/\/+$/, "") || "https://zendo.vn";
  const siteUrl = normalizeCanonicalOrigin(rawSiteUrl);
  ...
  return (
    <>
      ...
      {gtmId ? (
        <>
          <Script id="zendo-gtm-script" strategy="afterInteractive" src={`https://www.googletagmanager.com/gtm.js?id=${gtmId}`} />
      ...
      {fbPixel ? (
        <Script id="zendo-meta-pixel" strategy="afterInteractive">
          {`
              ...
              n.callMethod.apply(n,arguments):n.queue.push(arguments)};if(!f._fbq)n=f._fbq=n;
      ...
      {zaloPixel ? (
        <Script id="zendo-zalo-pixel" strategy="afterInteractive" src={`https://sp.zalo.me/plugins/sdk.js?pixel_id=${encodeURIComponent(zaloPixel)}`} />
      ) : null}
      {clarityId ? (
      ...
      <div className="relative z-10 flex min-h-[100dvh] w-full min-w-0 flex-col" suppressHydrationWarning>
              ...
              {trustItems.map((item, index) => (
                <div key={`${item.title}-${index}`} className="min-w-0 text-center">
                  ...
                  {item.description.trim() ? (
                    <p className="mt-0.5 text-[11px] leading-snug text-zinc-600 sm:text-xs">{item.description}</p>
                  ) : null}
                </div>
              ))}
            </div>
