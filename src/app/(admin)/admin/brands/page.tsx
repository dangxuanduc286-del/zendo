...
export const metadata: Metadata = {
  title: "Thương hiệu | Quản trị Zendo.vn",
  description: "Quản lý thương hiệu sản phẩm trong khu vực quản trị Zendo.vn.",
  robots: {
    index: false,
    follow: false,
  },
...
export default async function AdminBrandsPage(): Promise<JSX.Element> {
  const session = await getServerSession(authOptions);
  if (!session?.user?.id) {
    redirect("/admin/login?callbackUrl=/admin/brands");
  }
      ...
      <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#E2E8F0] bg-white p-5 shadow-sm sm:p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-[#64748B]">
            Quản lý thương hiệu sản phẩm, logo và SEO.
          </p>
        </div>
        <Link
