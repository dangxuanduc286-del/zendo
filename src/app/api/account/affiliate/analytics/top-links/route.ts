...
import { getAffiliateTopLinks } from "@/lib/affiliate-traffic-analytics";
import {
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const rl = await applyAnalyticsRateLimit({ key: `tl:${auth.customerId}`, windowMs: 10_000, max: 25 });
  if (!rl.ok) {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, message: "Không tải được top link affiliate." }, { status: 500 });
  }
}
