...
import { CTV_MEMBERSHIP_TIERS_CACHE_TAG } from "@/lib/ctv/ctv-membership-tier-repository";
...
export async function PUT(request: Request, ctx: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  await db.ctvMembershipTier.update({
    where: { id },
    data: {
      code: input.code,
      name: input.name,
