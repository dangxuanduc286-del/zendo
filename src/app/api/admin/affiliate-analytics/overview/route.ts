...
import { getAdminCampaignBreakdown, getAdminSystemOverview } from "@/lib/admin-affiliate-analytics";
import {
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    const [overview, campaigns] = await Promise.all([
      getAdminSystemOverview({ db, range, useCache: true }),
      getAdminCampaignBreakdown({ db, range, take: 15 }),
    ]);
    return NextResponse.json(
      { ok: true, overview, campaigns },
      { status: 200, headers: { "Cache-Control": "private, no-store" } },
    );
  } catch {
    return NextResponse.json({ ok: false, message: "Không tải được tổng quan affiliate." }, { status: 500 });
  }
}
