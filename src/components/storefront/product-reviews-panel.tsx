...
export default function ProductReviewsPanel({
  ...
  const emptyReviewMessage = reviewEmptyText.trim() || "Chưa có đánh giá nào";
    ...
    <section className="mt-8 sm:mt-10">
      <div className="rounded-2xl border bg-white p-4 shadow-sm sm:p-5" style={{ borderColor: cardBorderColor }}>
        <h2 className="flex items-center gap-2 text-xl font-semibold text-zinc-900">
          <span aria-hidden style={{ color: ratingColor }}>★</span>
          {reviewTitle}
        </h2>
        {hasReviews ? (
          <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[280px_1fr] lg:gap-6">
          ...
          <div className="mt-4 text-center">
            <p className="text-sm font-semibold text-zinc-900 sm:text-base">{emptyReviewMessage}</p>
            <p className="mt-1 text-sm text-zinc-600">
              0 {ratingLabel}
            </p>
            <div className="mt-2 text-xl tracking-wider" style={{ color: ratingColor }} aria-hidden>
              ★★★★★
            </div>
