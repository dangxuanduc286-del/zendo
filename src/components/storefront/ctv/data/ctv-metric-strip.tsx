...
import { CTV_V2_STATS_CELL, CTV_V2_STATS_GRID, CTV_V2_LABEL } from "../ctv-ui-tokens";
...
const COL_CLASS: Record<NonNullable<CtvMetricStripProps["columns"]>, string> = {
  2: "grid-cols-2 gap-2",
  3: "grid-cols-1 gap-2 sm:grid-cols-3 sm:gap-3",
  4: "grid-cols-2 gap-2 lg:grid-cols-4 lg:gap-3",
  6: "grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 xl:grid-cols-6",
};
      ...
      <dl className={`${CTV_V2_STATS_GRID} ${className}`} aria-label={ariaLabel}>
        {items.map((item) => (
          <div key={item.id} className={CTV_V2_STATS_CELL}>
            ...
            <dd className="mt-2 text-xl font-extrabold tabular-nums text-slate-900 sm:text-2xl">{item.value}</dd>
  ...
  return (
    ...
    >
      {items.map((item) => (
        <div
          key={item.id}
          className="min-w-0 overflow-hidden break-words rounded-xl border border-black/[0.04] bg-slate-50/80 p-3 transition-[transform,box-shadow] duration-[250ms] ease-out hover:-translate-y-0.5 hover:shadow-md max-lg:rounded-lg max-lg:p-3 sm:p-4"
        >
          <dt className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{item.label}</dt>
          <dd className="mt-2 text-lg font-extrabold tabular-nums text-slate-900">{item.value}</dd>
